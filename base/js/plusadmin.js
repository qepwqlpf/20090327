...
$(document).ready(function() {
		...
		$("body").append("<div id='contextMenu' class='contextMenu'><UL><LI id='cm_up'>移至顶层</LI><LI id='cm_top'>上移一层</LI><LI id='cm_under'>下移一层</LI><LI id='cm_down'>移至底层</LI></UL></div>");
		...
		$("#adminbarmenu").append("<div id='pdv_pagemeta' class='adminbutton1'>页面标题/参数</div>");
		...
		$("#admin_pageconset").append("<div class='pageconsetitem'>容器宽度：<select id='pagecontainwidth' ><option value='900'>默认(900PX)</option><option value='950'>门户(950PX)</option><option value='990'>宽幅(990PX)</option><option value='780'>窄幅(780PX)</option></select></div>");
		...
		$.ajax({
			...
			success: function(msg){
				...
				$("div.plusplanuse").click(function(){
					qus=confirm("应用排版方案将清除本页现有插件，建议先把当前排版保存为方案，以便恢复；方案中如果包含不可在本页使用的插件，将不会被插入。确定要使用新的排版方案吗？");
					if(qus!=0){
						var planid=this.id.substr(12);
						$.ajax({
							type: "POST",
							url: PDV_RP+"base/post.php",
							data: "act=plusplanuse&planid="+planid+"&pageid="+PDV_PAGEID,
							success: function(msg){
								if(msg=="OK"){
...
(function($){
	...
	$.fn.pageStyleSet = function(){
		...
		for(var i=0;i<$("#pagebgposition")[0].length;i++){
			if($("#pagebgposition")[0].options[i].value==$("body")[0].style.backgroundPosition){
...
(function($){
	$.fn.savePlus = function(ntc){
		...
		if(topbg==""){topbg=$("#topbg")[0].style.backgroundColor;}
		if(contentbg==""){contentbg=$("#contentbg")[0].style.backgroundColor;}
		if(bottombg==""){bottombg=$("#bottombg")[0].style.backgroundColor;}
		...
		$.ajax({
			...
			success: function(msg){
				if(msg=="OK"){
					if(ntc==1){
						$('#plusWindow').empty();
						$('#plusWindow').append("<div class='topBar'>提示信息<div class='pwClose'></div></div><div class='border'><div class='ntc'>排版信息已保存，按确定继续排版</div><div class='buttonzone'><input type='button' class='button' id='ok' value='确定' /></div></div>");
						$.blockUI({ message: $('#plusWindow'),css:{width:'300px'}}); 
						$('#ok').click(function() { 
							$.unblockUI(); 
						}); 
						$('.pwClose').click(function() { 
